...
class Instance(nn.Module):
    def __init__(self, in_channels, num_anchors, num_layers, onnx_export=False, root_dir='data'):
    ...
    def forward(self, inputs, text):
        ...
        for feat, bn_list in zip(inputs, self.bn_list):
            ...
            feat = feat.permute(0, 2, 3, 1)
            feat = feat.contiguous().view(feat.shape[0], feat.shape[1], feat.shape[2], self.num_anchors, 1)
            feat = feat.contiguous().view(feat.shape[0], -1, 1)
...
class EfficientDet(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        self.classifier = Classifier(in_channels=self.fpn_num_filters[self.compound_coef], num_anchors=num_anchors,
                                     ...
                                     num_layers=self.box_class_repeats[self.compound_coef])
        self.instance = Instance(in_channels=self.fpn_num_filters[self.compound_coef], num_anchors=num_anchors,
                                     num_layers=self.box_class_repeats[self.compound_coef])
    ...
    def forward(self, inputs):
        ...
        if self.training:
            return self.cost(instance, classification, regression, anchors, annot)
        else:
